...
public abstract class AbstractPubSubPlusTestCase {
    ...
    static {
        ...
        solace = new GenericContainer<>("solace/solace-pubsub-standard:latest").withExposedPorts(SMF_PORT, WEB_TRANSPORT,
                AMQP_PORT, MQTT_PORT, MQTT_WEB_PORT, REST_PORT, SEMP_PORT)
        ...
        LOGGER.info(
            String.format("Started Solace PubSub+ Docker Container, available on host [%s], SMF port [%d]", solace.getHost(),
                solace.getMappedPort(SMF_PORT)));
    }
